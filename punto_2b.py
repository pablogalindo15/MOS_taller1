...
# 6.a Tarea 1 sólo trabajador 1
def OnlyWorker1_Task1_rule(m, e):
    if 1 in m.T and e != 1:
        return m.x[e, 1] == 0
    return Constraint.Skip
model.OnlyWorker1_Task1 = Constraint(model.E, rule=OnlyWorker1_Task1_rule)

# 6.b Trabajador 2 no puede hacer tarea 3
def Worker2_Not_Task3_rule(m):
    if 2 in m.E and 3 in m.T:
        return m.x[2, 3] == 0
    return Constraint.Skip
model.Worker2_Not_Task3 = Constraint(rule=Worker2_Not_Task3_rule)
...
if __name__ == "__main__":
    ...
    # Etiquetas de celdas opcionales (marcar las asignaciones)
